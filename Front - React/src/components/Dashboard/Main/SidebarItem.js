...
const SidebarItem = ({ icon, text, active, alert, expanded }) => {
  ...
  return (
    ...
    >
      {icon}
      <span className={`overflow-hidden transition-all ${contextExpanded ? "w-52 ml-3" : "w-0"}`}>{text}</span>
      {alert && (
        <div className={`absolute right-2 w-2 h-2 rounded bg-indigo-400 ${contextExpanded ? "" : "top-2"}`} />
      )}
      {!contextExpanded && (
        <div
